...
### Deferred surface
- External raw-tool host adoption outside the active supported runtime surface remains out of scope for this package. This matches the current package scope: hooks enforce the host runtime's own tool surfaces, not arbitrary external executors.
...
- External raw-tool endpoint adoption is follow-up hardening, not part of the current completion claim.

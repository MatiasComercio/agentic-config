...
Operational default: include `--stream` for wrapper launches so live child JSONL events appear in the background Bash task output. Omit it only for low-noise automation. Startup silence is warning-only by default because workers can be legitimately silent before the first model/tool event; use `--startup-warn-after N` to tune or disable that diagnostic. Extended output silence is fail-closed for `pi-bash.py` stream launches: the default idle timeout is 600 seconds, `--idle-timeout N` tunes it, and `--idle-timeout 0` disables it only for explicitly justified cases.

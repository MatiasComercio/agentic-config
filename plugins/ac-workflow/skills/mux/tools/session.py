...
The mux-active marker is kept for external observability and deactivation signaling,
but is NOT the mechanism that activates enforcement hooks.
...
def activate_mux_enforcement(session_dir: Path) -> Path | None:
    ...
    Hook enforcement is now skill-scoped (mux.md / mux-subagent.md frontmatter),
    not triggered by this marker.

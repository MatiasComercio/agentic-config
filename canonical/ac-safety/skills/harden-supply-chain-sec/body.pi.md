...
## Section 12: Hardening B -- Dependency Security Review Worker (Gate 3)
...
This will launch a security review worker through the current runtime subagent tool that:
...
Use the `subagent` tool to launch a review worker with the following prompt:
...
If the subagent fails: WARN with error details but continue to Gate 4.
